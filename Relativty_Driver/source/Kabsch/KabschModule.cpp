...
static void TrackMarkers3D(ControlState* state)
{
	...
	{ // TODO: Replace with expected marker after initial detection
		auto pose = detectMarker3D(&*markerTemplate, state->tracking.points3D, state->tracking.conflicts, state->tracking.nonconflictedCount, state->tracking.sigmaError);
		if (std::get<2>(pose) > 0)
		{ // Found a pose
			state->tracking.poses3D.push_back(std::get<0>(pose));
			state->tracking.posesMSE.emplace_back(std::get<1>(pose), std::get<2>(pose));
		}
	}
}
...
void generateLookupTables(MarkerTemplate3D* marker3D)
{
	...
	{
		//PRINT_ERR("Marker has more marker points (%d) than maximum code has been compiled with (%d)", (int)marker3D->points.size(), MAX_MARKER_POINTS);
	}
...
void castRays(const std::vector<Eigen::Vector2f>& points2D, const Camera& camera, std::vector<Ray>& rays3D)
{
	...
	Eigen::Vector3f camPos = camera.transform.translation();
	...
	for (int i = 0; i < points2D.size(); i++)
	{
		...
		Eigen::Vector3f dir = (end - camPos).normalized();
		Eigen::Vector3f start = camPos + dir; // On camera pos will be problematic for viz
		rays3D.push_back({ start, dir, 0 });
	}
}
...
int triangulateRayIntersections(std::vector<std::vector<Ray>*>& rayGroups, std::vector<TriangulatedPoint>& points3D, std::vector<std::vector<int>>& conflicts, float maxError, float minError)
{
	...
	{
		//PRINT_ERR("Exceeded maximum camera code of %d! Recompile needed.", MAX_CAMERA_COUNT);
...
#define IXNUM(ix) (int)(ix == NULL? -1 : (((intptr_t)ix-(intptr_t)intersections.data())/(sizeof(Intersection))))
		...
		for (int j = i + 1; j < groupCnt; j++)
		{
			...
			for (int v = 0; v < rays1->size(); v++)
			{
				...
				for (int w = 0; w < rays2->size(); w++)
				{
					...
					//					PRINT("Intersection with error %f", std::sqrt(errorSq));
				}
			}
		}
	}
	//	PRINT("Got %d intersections!", (int)intersections.size());
	...
	for (int i = 0; i < intersections.size(); i++)
	{
		...
		if (ix->merge != NULL)
		{
			//			PRINT("------ Skipping merged intersections %d on rays %d %d %d!", i, RAYNUM(ix->rays, 0), RAYNUM(ix->rays, 1), RAYNUM(ix->rays, 2));
			continue;
		}
		...
		/*		PRINT("------ Intersections %d on rays %d %d %d!", i, RAYNUM(ix->rays, 0), RAYNUM(ix->rays, 1), RAYNUM(ix->rays, 2));
				std::stringstream cfDbg;
				for (int j = 0; j < potentialMergers.size(); j++)
					cfDbg << IXNUM(potentialMergers[j]) << " - ";
				PRINT("Potential mergers: %s", cfDbg.str());
		*/
...
// Begin OOR fix
			...
			/*			std::stringstream rayDbg;
						...
						PRINT("Involved Rays: %s", rayDbg.str());
			*/
			...
			for (int i = 0; i < potentialMergers.size(); i++)
			{
				...
				{ // Accept as merger, probably out of error range
					...
					//					PRINT("Added intersection %d to merge because of shared rays!", IXNUM(potentialMergers[i]));
				}
			}
			// Emd OOR fix
	...
	for (int i = 0; i < groupCnt; i++)
	{
		...
		{
			//			PRINT("Group %d - Ray %d: %d intersections!", i, j, rays->at(j).intersectionCount);
...
/*	int ixCnt = 0;
	...
	PRINT("Merge Status: %d single intersections, plus %d merged intersections!", ixCnt, (int)mergedIntersections.size());
	PRINT("Conflict Status: %d certain points, plus %d uncertain in a total of %d conflicts!", pointCount, conflictedPointCount, conflictCount);
*/
...
void detectMarker3D(const MarkerTemplate3D* marker3D, const std::vector<TriangulatedPoint>& points3D, const std::vector<std::vector<int>>& conflicts, int nonconflictedCount, std::vector<MarkerCandidate3D>& candidates, float sigmaError, bool quickAssign)
{
	...
	float maxRelevantDistance = marker3D->relationDist.size() == 0 ? 0.0f :
		marker3D->relationDist[marker3D->relationDist.size() - 1].distance * 1.2f + 1.0f;
		...
		std::pair<std::vector<PointRelation>::const_iterator, std::vector<PointRelation>::const_iterator> mkRelBaseRange)
	{
		...
		{ // Iterate over all matching base relation candidates
			...
			//			PRINT("|  _ - Mk Base Relation (%d - %d) of length %f!", mkRelBase->pt1, mkRelBase->pt2, mkRelBase->distance);
...
//						PRINT("|  |  _ - Mk Arm Relation (%d - %d) with length %f (error %f +- %f)!", mkRelArm->pt1, mkRelArm->pt2, mkRelArm->distance, std::abs(trRelArm->distance - mkRelArm->distance), armError);
						...
						{
							//							PRINT("|  |  |    - Expected angle %f but is %f!", mkAngle/PI*180.0f, trAngle/PI*180.0f);
							continue;
						}
						...
						// Debug
//						PRINT("|  |  |  _ - Candidate %d: (%d->%d), (%d->%d), (%d->%d)", (int)candidates.size()-1, trPtA, mkPtA, trPtJ, mkPtJ, trPtB, mkPtB);
/*						PRINT("|  |  |  _ - Candidate %d: (%d->%d), (%d->%d), (%d->%d); Angle %f %+f (+-%f); 1-2 %f %+f (+-%f); 2-3 %f %+f (+-%f)", (int)candidates.size()-1,
...
// Calculate estimated pose of the three points
						...
						for (int j = 0; j < points3D.size(); j++)
						{
							...
							for (int k = 0; k < marker3D->points.size(); k++)
							{
								...
								{
									//									PRINT("|  |  |  |    - Added (%d - %d) - dist %f, max %f!", j, k, std::sqrt(errorSq), errorLimit);
								...
								{
									//									PRINT("|  |  |  |    - Rejected (%d - %d) - dist %f, max %f!", j, k, std::sqrt(errorSq), errorLimit);
								}
							}
						}
					...
					{ // Found potential arm with same distance
//						PRINT("|  |  _ - Dropped Mk Arm Relation (%d - %d) with length %f (error %f +- %f)!", mkRelArm->pt1, mkRelArm->pt2, mkRelArm->distance, std::abs(trRelArm->distance - mkRelArm->distance), armError);
					}
				}
			}
		}
	...
	for (int i = 0; i < points3D.size(); i++)
	{
		...
		for (int j = 0; j < trRelations[i].size(); j++)
		{
			...
			//			PRINT("Tr Base Relation (%d - %d) of length %f += %f: %d potential matches in marker!", trRelBase->pt1, trRelBase->pt2, trRelBase->distance, baseError, std::distance(mkRelBaseRange.first, mkRelBaseRange.second));
			...
			for (int k = 0; k < trRelations[i].size(); k++)
			{
				...
				//				PRINT("_ - Tr Arm Relation (%d - %d) with length %f += %f!", trRelArm->pt1, trRelArm->pt2, trRelArm->distance, armError);
...
/**
	...
	*/
void matchTrackedPoses(const std::vector<Eigen::Isometry3f>& currentPose, const std::vector<Eigen::Isometry3f>& lastPose, const std::vector<Eigen::Vector3f>& lastDir, const std::vector<float>& lastRot, std::vector<int>& matching, float maxTemporalStaticErrorT, float maxTemporalStaticErrorR, float maxTemporalDynamicErrorT, float maxTemporalDynamicErrorR)
{
	...
	for (int i = 0; i < lastPose.size(); i++)
	{
		...
		float acceptedPosErr = lastDir[i].norm() * maxTemporalDynamicErrorT * t + maxTemporalStaticErrorT;
		float acceptedPosErrSq = acceptedPosErr * acceptedPosErr;
		float acceptedRotErr = lastRot[i] * maxTemporalDynamicErrorR * t + (maxTemporalStaticErrorR / 180.0f * PI);
		...
		for (int j = 0; j < currentPose.size(); j++)
		{
			...
			{
				//PRINT("Discarded temporal match %d/%d with error %f > %f", j, i, std::sqrt(posErrSq), std::sqrt(acceptedPosErrSq));
			}
		}
	}
